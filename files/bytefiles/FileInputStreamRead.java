/*
    ...
    Constructors used for FileInputStream class
        ...
        the file represented by parameter file, in the file system.

        > FileInputStream(String name) - creates a FileInputStream by opening a connection to an actual file,
        the file named by the parameter name, in the file system.
*/
...
public class FileInputStreamRead {
    public static void main(String[] args) {
        try {
            ...
            // read() - Reads a byte of data, if EOF reached, returns -1
            // outputs 104, i.e., h of hello in test.txt
            int data = fin.read();
            System.out.println(data);

            /*
             * while ((data = fin.read()) != -1) { System.out.println((char) data); }
             */

            byte b[] = new byte[5];
            // read(byte b[], int offset, int length) - reads upto length bytes of data from
            // this input stream into an array of type byte, starting from index offset
            fin.read(b, 1, 3);
            for (byte k : b) {
